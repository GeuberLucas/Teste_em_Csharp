...
namespace Teste_Csharp
{
    public class MyUserStore : IUserStore<MyUser>,IUserPasswordStore<MyUser>
    {
        public async Task<IdentityResult> CreateAsync(MyUser user, CancellationToken cancellationToken)
        {
            ...
            {
                await connection.ExecuteAsync("insert into dbo.Users([Id]," +
                    "[UserName]," +
                    "[NormalizedUserName]," +
                    "[PasswordHash])" +
        ...
        {
            SqlConnection connection = new("Integrated Security=SSPI;" +
                "Persist Security Info=False;Initial Catalog=Testes;" +
                @"Data Source=TISUPERSIMPLES\SQLEXPRESS");
            connection.Open();
